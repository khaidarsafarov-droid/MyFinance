...
private fun authenticateToken(
    ...
    if (config.environment == AppEnvironment.TEST && config.testAuthEnabled && token.startsWith("test.")) {
        val id =
            runCatching { UUID.fromString(token.removePrefix("test.")) }.getOrNull() ?: return null
    ...
    route("/sync") {
        ...
        put("/snapshot") {
            ...
            if (stored.accepted && sourceDeviceId != null) {
                val tokens =
                    repositories.pushTokens.listForUser(user.id, sourceDeviceId).map { it.token }
...
private fun io.ktor.server.routing.Route.mediaRoutes(
    config: AppConfig,
    ...
    route("/media") {
        get {
            ...
            val records =
                dependencies.repositories.media.list(user.id, since, kind, MAX_MEDIA_LIST_ITEMS)
            val items = records.map { record ->
                record.toDownloadContract(
                    config,
        ...
        post("/upload-url") {
            ...
            if (request.sizeBytes !in 1..config.maxUploadBytes) {
                throw ApiException(
                    HttpStatusCode.BadRequest,
                    "invalid_size",
                    "Upload size is outside the allowed range"
                )
            }
            ...
            if (record.deletedAt != null) {
                throw ApiException(
                    ...
                    "media_deleted",
                    "This client media id was deleted"
                )
            }
            if (
...
private fun io.ktor.server.routing.Route.localMediaUpload(
    config: AppConfig,
    dependencies: AppDependencies
) {
...
private fun io.ktor.server.routing.Route.localMediaDownload(
    config: AppConfig,
    dependencies: AppDependencies
) {
