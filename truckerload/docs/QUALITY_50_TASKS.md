...
12. ✅ Log camera/scanner exceptions instead of empty `catch (_)` (camera + location; scanner remaining soft)
13. ✅ Nullable location instead of `0.0,0.0` fake coords
14. ✅ Camera-for-load `onFinished` → `popBackStack()` (load-scoped returns to detail)
...
30. ✅ Widget stats: use week loads (+ SQL yield), not full hydrate
