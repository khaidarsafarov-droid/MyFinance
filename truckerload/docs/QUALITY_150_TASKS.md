...
On current main: Telegram permanently skips failed updates; swipe-delete is immediate; load delete orphans photos/scans/files; backup restore is non-transactional; tax per-diem uses `loads.size`; CDC trip IDs are case-sensitive; Drive/widget prefs cross accounts; overnight “paging” is unused by Home UI.
