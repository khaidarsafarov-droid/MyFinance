...
Вход: **только Google Auth** (на iOS позже — Sign in with Apple / iCloud).
...
4. Откройте **Truck Log / TruckerLoad** и нажмите **Войти через Google**.

Обновления: ставьте новый APK поверх старого (тот же ключ подписи). Данные Room сохраняются
для того же Google-аккаунта на устройстве.
