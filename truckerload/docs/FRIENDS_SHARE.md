...
Release APK **не** содержит `TELEGRAM_BOT_TOKEN` / `CEREBRAS_API_KEY` (BuildConfig
очищается при `release`). Друг добавляет **свой** бот в Настройках и привязывает чат
через OTP-код (Настройки → Telegram → «Код привязки» → `/start КОД` в боте).
