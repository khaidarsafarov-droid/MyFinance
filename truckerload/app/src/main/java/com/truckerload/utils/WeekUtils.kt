    ...
    return load.stops
        ...
        .mapNotNull {
            parseDateFromScheduledTime(it.scheduledTime, yearHint, trustDefaultYear = yearHint != null)
                ?.let { del -> bumpIsoYearIfBefore(del, puDate) }
        }
        .maxOrNull()
}
...
fun getLoadDateRange(load: Load): Set<String> {
    ...
    val yearHint = load.date.take(4).toIntOrNull()
    val puDate = getPickUpDate(load)
    val stopDates = load.stops.mapNotNull { stop ->
        parseDateFromScheduledTime(stop.scheduledTime, yearHint, trustDefaultYear = yearHint != null)
            ?.let { parsed ->
                if (stop.type == StopType.DEL) bumpIsoYearIfBefore(parsed, puDate) else parsed
            }
    }
