...
/**
 * If [dateIso] falls before [anchorIso], bump the year by 1.
 * Relay New Year trips (PU `12/30` → DEL `01/02`) need this when MM/DD was anchored to the PU year.
 */
fun bumpIsoYearIfBefore(dateIso: String, anchorIso: String?): String {
    val date = dateIso.takeIf { it.length >= 10 }?.take(10) ?: return dateIso
    val anchor = anchorIso?.takeIf { it.length >= 10 }?.take(10) ?: return date
    if (date >= anchor) return date
    val year = date.take(4).toIntOrNull() ?: return date
    return "%04d%s".format(year + 1, date.drop(4))
}
