...
/** Ephemeral friend statuses (text / photo / voice stories). */
class StatusRepository(
    ...
    }.getOrElse { SocialResult.Error(appContext.socialError(R.string.social_error_create_status, it), it) }

    suspend fun markStatusViewed(statusId: String): SocialResult<Unit> = runCatching {
        driverStatusDao.markViewed(statusId)
        SocialResult.Success(Unit)
    }.getOrElse { SocialResult.Error(appContext.socialError(R.string.social_error_mark_viewed, it), it) }
