...
/**
 * Room-backed [ProfileRepository]: identity, avatar, and social-graph ops.
 *
 * Graph helpers live in [ProfileSocialGraph] so this file stays near the 350-line ideal.
 */
class ProfileRepositoryImpl(
    ...
    private val socialGraph: ProfileSocialGraph,
    context: Context,
) : ProfileRepository {
    ...
    override suspend fun blockUser(blockedId: String) = socialGraph.blockUser(blockedId)
    override suspend fun unblockUser(blockedId: String) = socialGraph.unblockUser(blockedId)
    override suspend fun isBlocked(targetId: String) = socialGraph.isBlocked(targetId)
    override fun watchIsBlocked(targetId: String) = socialGraph.watchIsBlocked(targetId)
    override suspend fun followDriver(targetId: String) = socialGraph.followDriver(targetId)
    override suspend fun unfollowDriver(targetId: String) = socialGraph.unfollowDriver(targetId)
    override fun watchIsFollowing(targetId: String) = socialGraph.watchIsFollowing(targetId)
    override fun watchPeer(peerId: String) = socialGraph.watchPeer(peerId)
    override suspend fun getPeer(peerId: String) = socialGraph.getPeer(peerId)

    override suspend fun displayNameOrBlank(): String =
