...
/**
 * Deprecated facade over domain social repositories.
 *
 * ViewModels keep injecting this type in Phase 3.1; migrate callers to the
 * specific repositories, then remove this class.
 */
@Deprecated("Use specific repositories. Will be removed in Phase 3.6")
@UserScope
class SocialRepository @Inject constructor(
    ...
    private val sync: SocialSyncCoordinator,
) {
    suspend fun ensureInitialized() = sync.ensureInitialized()
    ...
    fun watchPublicGroups(): Flow<List<SocialChat>> = group.watchPublicGroups()
    ...
    fun recommendGroups(): Flow<List<SocialChat>> = group.recommendGroups()
    ...
    suspend fun createPrivateChat(peerName: String): SocialResult<String> =
        chat.createPrivateChat(peerName)
    ...
    suspend fun markStatusViewed(statusId: String): SocialResult<Unit> =
        status.markStatusViewed(statusId)
    ...
    suspend fun sendImageMessage(
        ...
        caption: String,
        senderName: String,
    ): SocialResult<Unit> = chat.sendImageMessage(chatId, bitmap, caption, senderName)
    ...
    suspend fun sendVoiceMessage(
        ...
        durationMs: Long,
        senderName: String,
    ): SocialResult<Unit> = chat.sendVoiceMessage(chatId, audioFile, durationMs, senderName)

    suspend fun createPhotoStatus(
        bitmap: Bitmap,
        displayName: String,
        caption: String = "",
    ): SocialResult<Unit> = status.createPhotoStatus(bitmap, displayName, caption)

    suspend fun createVoiceStatus(
        audioFile: File,
        durationMs: Long,
        displayName: String,
    ): SocialResult<Unit> = status.createVoiceStatus(audioFile, durationMs, displayName)
    ...
    ): Flow<List<LeaderboardEntry>> = profile.watchLeaderboard(category)
    ...
    /** Retained for callers that only need the media boundary (tests / future migration). */
    fun mediaRepository(): MediaRepository = media

    companion object {
        const val MESSAGE_PAGE_SIZE = ChatRepository.MESSAGE_PAGE_SIZE
        const val STATUS_TTL_MS = StatusRepositoryImpl.STATUS_TTL_MS
        const val WEEKLY_CHALLENGE_ID = ProfileChallengeHelper.WEEKLY_CHALLENGE_ID
    }
}
