...
class GroupRepositoryImpl(
    ...
    private val recommendations: RecommendationService = RecommendationService(),
    context: Context,
) : GroupRepository {
    ...
    override fun recommendGroups(chats: Flow<List<SocialChat>>): Flow<List<SocialChat>> =
        chats.map { recommendations.recommendGroups(it) }.flowOn(Dispatchers.IO)
    ...
    override suspend fun createGroupChat(name: String, category: String): SocialResult<String> =
        ...
        }.getOrElse { SocialResult.Error(appContext.socialError(R.string.social_error_create_group, it), it) }
    ...
    }.getOrElse { SocialResult.Error(appContext.socialError(R.string.social_error_leave_group, it), it) }
    ...
    override suspend fun joinGroup(chatId: String, displayName: String): SocialResult<Unit> = runCatching {
        ...
        SocialResult.Success(Unit)
    }.getOrElse { SocialResult.Error(appContext.socialError(R.string.social_error_join_challenge, it), it) }
