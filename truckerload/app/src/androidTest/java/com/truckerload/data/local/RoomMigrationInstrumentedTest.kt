...
class RoomMigrationInstrumentedTest {
    ...
    fun migrate6To36_smoke() {
        ...
        val db = helper.runMigrationsAndValidate(testDb, 36, false, *ALL_MIGRATIONS_FROM_V6)
    ...
    @Throws(IOException::class)
    fun migrate35To36() {
        helper.createDatabase(testDb, 35).apply { close() }
    ...
    @Throws(IOException::class)
    fun migrate36To37() {
        helper.createDatabase(testDb, 36).apply { close() }
    ...
    @Throws(IOException::class)
    fun migrate37To38() {
        helper.createDatabase(testDb, 37).apply { close() }
    ...
    @Throws(IOException::class)
    fun migrate40To41() {
        helper.createDatabase(testDb, 40).apply { close() }
     ...
     * (historical schemas are not committed; only 28–41 are).
     */
