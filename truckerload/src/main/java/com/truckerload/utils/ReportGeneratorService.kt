...
/**
 * Generates professional PDF reports from TruckerLoad data.
...
class ReportGeneratorService(private val context: Context) {
    ...
    /**
     * Generates PDF and saves it to public Downloads/TruckerLoad/Reports.
    ...
    suspend fun generatePdfAndSaveToStorage(params: ReportParams): StorageHelper.SaveResult? = withContext(Dispatchers.IO) {
        try {
            ...
            val fileName = "TruckerLoad_Report_${safeLabel}_${System.currentTimeMillis()}.pdf"
            val storageHelper = StorageHelper(context)
            storageHelper.saveToPublicDownloads(fileName, "TruckerLoad/Reports", "application/pdf") { out ->
                generatePdfToStream(params, out)
            }
                ?: run {
                    val file = storageHelper.saveToAppStorage(fileName, "Reports") { generatePdfToStream(params, it) }
                    StorageHelper.SaveResult(storageHelper.getShareableUri(file), "TruckerLoad/Reports/$fileName")
                }
        } catch (e: Exception) {
    ...
    private fun generatePdfToStream(params: ReportParams, out: java.io.OutputStream) {
        ...
        try {
            var y = margin.toFloat()
                ...
                // Header
                drawText("TruckerLoad", titlePaint)
