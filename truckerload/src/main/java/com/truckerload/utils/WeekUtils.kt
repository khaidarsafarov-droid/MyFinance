...
fun getWeekRange(weekNumber: Int, year: Int): Triple<String, String, String> {
    ...
    val label = "${monthNames[startMonth]} $startDay – ${monthNames[endMonth]} $endDay, $startYear"
...
/** Короткая подпись недели: "24 фев — 2 мар" (учитывает firstDayOfWeek из локали). */
fun getWeekLabelShort(weekNumber: Int, year: Int): String {
    ...
    val monthNamesShort = listOf("", "янв", "фев", "мар", "апр", "май", "июн", "июл", "авг", "сен", "окт", "ноя", "дек")
    ...
    return "$startD ${monthNamesShort.getOrElse(startM) { "" }} — $endD ${monthNamesShort.getOrElse(endM) { "" }}"
}
